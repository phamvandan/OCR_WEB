...
def get_string_from_image(box, image):
    (x, y, w, h) = box
...
def layout_document(image, document):
    ...
    for k, temp in enumerate(list_result):
        ...
        for index,(x,y,w,h) in enumerate(temp):
            if (x,y,w,h) == (0,0,0,0):
                ...
                p.alignment = align
                continue
            string = get_string_from_image((x, y, w, h), image)
            p = row_cells[index].add_paragraph(string)
            p.alignment = align
    return list_result, document


def get_text_layout(list_result, list_big_box, img, doc_name):
    ...
    for index, y1 in enumerate(list_y_coord):
        ...
        if (y1, y2) not in big_box_temp:
            ...
            if temp < 2:
                continue
            layout_document(crop, document)
            result.append(pytesseract.image_to_string(crop,
                                                      lang='vie') + "\n")  # for get text
        else:
            index = 0
...
def get_text(list_result, list_big_box, img):
    result = []
    (height, _) = img.shape[:2]
    if len(list_big_box) == 0:
        result.append(pytesseract.image_to_string(img, lang='vie') + "\n")
